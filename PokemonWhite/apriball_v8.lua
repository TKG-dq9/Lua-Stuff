...
local items = {
   -- ID, name, bag sprite NCGR (a/0/2/5)
   {0x01EC, "Fast Ball", 723},
   {0x01ED, "Level Ball", 717},
   {0x01EE, "Lure Ball", 715},
   {0x01EF, "Heavy Ball", 721},
   {0x01F0, "Love Ball", 727},
   {0x01F1, "Friend Ball", 725},
   {0x01F2, "Moon Ball", 719},
   {0x01F3, "Sport Ball", 731},
   {0x01F4, "Park Ball", 729},
   {0x0005, "Safari Ball"}
}
...
local speed = {
   ...
   {0x01EA, 100}, -- Manaphy
   {0x01EC, "100 (Land), 127 (Sky)"}, -- Shaymin
   {0x01EE, 100}, -- Victini
   ...
   {0x0269, 145}, -- Accelgor
   {0x0182, "150 (Normal/Attack), 90 (Defense), 180 (Speed)"}, -- Deoxys
   {0x0123, 160}, -- Ninjask
}
...
memory.registerexec(0x021cbb5c, function()
   ...
   -- set tiles to bag sprite NCGR file
   for i = 1, 9 do
      if r3 == items[i][1] then
         ncgr = items[i][3]
      end
...
memory.registerexec(0x021cbc08, function()
   ...
   if flagApri then
      ...
      flagApri = false
      if debugger and catchRateMod then
         print("Modified catch rate: " .. catchRateMod)
      end
   end
end)

memory.registerexec(0x021FC214, function()
   if flagSprite and ncgr then
      local tiles = ncgr
      local palette = ncgr + 1
      memory.setregister("r1", 25) -- a/0/2/5
      memory.setregister("r2", tiles) -- .NCGR
      memory.setregister("r3", palette) -- .NCLR
      if debugger then
         print("Sprite: loaded path a/0/2/5 (5_" .. tiles .. ".RGCN/5_" .. palette .. ".RLCN)")
         print("\n")
      end
   end
end)

memory.registerexec(0x02201be4, function()
   if flagSprite then
      memory.setregister("r1", 1)
      memory.setregister("r2", 0)
   end
end)
