...
irq_handler:
    ...
    @ Check VCOUNT to see if we are in the VBlank period (Scanlines 192-262)
    ldr r0, =0x04000006     @ VCOUNT Address
    ldrh r1, [r0]           @ Read 16-bit VCOUNT
    cmp r1, #192          @ Check if >= 192 (0xC0)
    blt exit_irq            @ If less than 192, screen is drawing; skip payload
    ...
    @ ====================================================
    @ Fix flickering by patching BG2CNT and BG0CNT
    @ ====================================================

    @ 1. Non-destructive BG2CNT Screen Base Patch
    ldr r0, =0x0400000C     @ BG2CNT Address
    ldrh r1, [r0]           @ Read current BG2CNT
    
    ldr r2, =0x1F00         @ Mask for bits 8-12 (Screen Base Block)
    bic r1, r1, r2          @ Clear current Screen Base Block
    
    ldr r2, =0x1E00         @ Set Screen Base Block to 30 (0x0600F000)
    orr r1, r1, r2
    strh r1, [r0]           @ Write preserved BG2CNT back

    @ 2. Stabilize BG0 Layer Priority
    ldr r0, =0x04000008     @ BG0CNT Address
    mov r1, #0x03
    strb r1, [r0]           @ Force low byte to 0x03
